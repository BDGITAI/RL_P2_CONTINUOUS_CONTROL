...
#######################################################################
#  This file is modified to include a new task : continuouscontrol    #
#  It enables the interface with the unity environment in the context #
#  of the continuous control project part of the RL nanodegree from   #
#  Udacity. Base file was developed for openai and atari environment. #
#                                                                     #
#  The new task uses the Unity class included in unity.py             #
#######################################################################
...
#
# New class added part of the P2_continuous control project
#
class ContinuousControl(BaseTask):
	# Class constructor requires the Unity environment as a parametre
	# The way the original code is structured triggered make of the
	# environment which is not compatible with Unity. If one Unity env runs
	# then a timeout is received during the second creation. To avoid this
	# the environment is create outside the task to control its creation
    def __init__(self, env, log_dir=None):
        BaseTask.__init__(self)
		# attributes taken from the pendulum task (above)
        self.name = 'ContinuousControl'
        self.env = env         
        self.state_dim = self.env.state_dim
        self.action_dim = self.env.action_dim
		# no logger task create. The monitor would require modifications
		# as it is based on the openai gym environment. Keeping this call 
		# created issues during execution
		## Todo create new monitor if required

	# step function identical to parent step function
    def step(self, action):
        next_state, reward, done, info = self.env.step(action)
        if done:
                next_state = self.env.reset()
